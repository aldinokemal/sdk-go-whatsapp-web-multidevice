/*
WhatsApp API MultiDevice

This API is used for sending whatsapp via API

API version: 3.3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ErrorBadRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ErrorBadRequest{}

// ErrorBadRequest struct for ErrorBadRequest
type ErrorBadRequest struct {
	// HTTP Status Code
	Code *int32 `json:"code,omitempty"`
	// Detail error message
	Message *string `json:"message,omitempty"`
	// additional data
	Results map[string]interface{} `json:"results,omitempty"`
}

// NewErrorBadRequest instantiates a new ErrorBadRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewErrorBadRequest() *ErrorBadRequest {
	this := ErrorBadRequest{}
	return &this
}

// NewErrorBadRequestWithDefaults instantiates a new ErrorBadRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewErrorBadRequestWithDefaults() *ErrorBadRequest {
	this := ErrorBadRequest{}
	return &this
}

// GetCode returns the Code field value if set, zero value otherwise.
func (o *ErrorBadRequest) GetCode() int32 {
	if o == nil || isNil(o.Code) {
		var ret int32
		return ret
	}
	return *o.Code
}

// GetCodeOk returns a tuple with the Code field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ErrorBadRequest) GetCodeOk() (*int32, bool) {
	if o == nil || isNil(o.Code) {
		return nil, false
	}
	return o.Code, true
}

// HasCode returns a boolean if a field has been set.
func (o *ErrorBadRequest) HasCode() bool {
	if o != nil && !isNil(o.Code) {
		return true
	}

	return false
}

// SetCode gets a reference to the given int32 and assigns it to the Code field.
func (o *ErrorBadRequest) SetCode(v int32) {
	o.Code = &v
}

// GetMessage returns the Message field value if set, zero value otherwise.
func (o *ErrorBadRequest) GetMessage() string {
	if o == nil || isNil(o.Message) {
		var ret string
		return ret
	}
	return *o.Message
}

// GetMessageOk returns a tuple with the Message field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ErrorBadRequest) GetMessageOk() (*string, bool) {
	if o == nil || isNil(o.Message) {
		return nil, false
	}
	return o.Message, true
}

// HasMessage returns a boolean if a field has been set.
func (o *ErrorBadRequest) HasMessage() bool {
	if o != nil && !isNil(o.Message) {
		return true
	}

	return false
}

// SetMessage gets a reference to the given string and assigns it to the Message field.
func (o *ErrorBadRequest) SetMessage(v string) {
	o.Message = &v
}

// GetResults returns the Results field value if set, zero value otherwise.
func (o *ErrorBadRequest) GetResults() map[string]interface{} {
	if o == nil || isNil(o.Results) {
		var ret map[string]interface{}
		return ret
	}
	return o.Results
}

// GetResultsOk returns a tuple with the Results field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ErrorBadRequest) GetResultsOk() (map[string]interface{}, bool) {
	if o == nil || isNil(o.Results) {
		return map[string]interface{}{}, false
	}
	return o.Results, true
}

// HasResults returns a boolean if a field has been set.
func (o *ErrorBadRequest) HasResults() bool {
	if o != nil && !isNil(o.Results) {
		return true
	}

	return false
}

// SetResults gets a reference to the given map[string]interface{} and assigns it to the Results field.
func (o *ErrorBadRequest) SetResults(v map[string]interface{}) {
	o.Results = v
}

func (o ErrorBadRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ErrorBadRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Code) {
		toSerialize["code"] = o.Code
	}
	if !isNil(o.Message) {
		toSerialize["message"] = o.Message
	}
	if !isNil(o.Results) {
		toSerialize["results"] = o.Results
	}
	return toSerialize, nil
}

type NullableErrorBadRequest struct {
	value *ErrorBadRequest
	isSet bool
}

func (v NullableErrorBadRequest) Get() *ErrorBadRequest {
	return v.value
}

func (v *NullableErrorBadRequest) Set(val *ErrorBadRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableErrorBadRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableErrorBadRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableErrorBadRequest(val *ErrorBadRequest) *NullableErrorBadRequest {
	return &NullableErrorBadRequest{value: val, isSet: true}
}

func (v NullableErrorBadRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableErrorBadRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


